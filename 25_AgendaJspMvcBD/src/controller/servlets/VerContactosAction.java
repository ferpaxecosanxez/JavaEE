...
@WebServlet("/VerContactosAction")
public class VerContactosAction extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		for (Contacto contacto : contactos) {
			...
			Optional<Telefono> optional = telefonos.stream().filter(t -> t.getIdContacto() == contacto.getId())
					.findFirst();
			...
			if (optional.isPresent()) {
				...
				contactoAgenda.setId(contacto.getId());
		...
		// resultados.
		request.getRequestDispatcher("Controlador?op=toVisualizaContactos").forward(request, response);
	}
}
