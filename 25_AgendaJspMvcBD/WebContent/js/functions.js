...
function validaFormulario(form) {
	...
	for (i = 0; i < form.childNodes.length; i++) {
		...
		// Capturar <div> que dentro tendrá opciones Checks, Radios, Etc.
		...
		// Capturar <select> que dentro tendrá opciones ComboBox.	
		} else if (itemForm.tagName != null && itemForm.tagName == "SELECT") {
			var classCss = itemForm.attributes.class.nodeValue;
			optionName = itemForm.name;
			
			// Opciones Comboox.
			if (classCss.indexOf("opcionesComboBox") != -1
					&& itemForm.value == "0") {// 0 porque lo hemos definido así en el formulario HTML.
				error = true;
				msg += "Debe seleccionar una opci\u00F3n correspondiente a: "
					+ optionName + ".\n\n";
			}
